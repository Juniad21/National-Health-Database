...
<div x-data="{
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div @click="selectDoctor(doc)" 
                        ...
                        <div class="flex gap-5 relative z-10">
                            <div class="w-16 h-16 bg-slate-100 rounded-2xl flex-shrink-0 flex items-center justify-center text-teal-600 font-black text-2xl shadow-inner group-hover:bg-teal-600 group-hover:text-white transition-all">
                                <span x-text="doc.name.split(' ')[1][0]"></span>
                            </div>
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-1 text-xs text-slate-400 font-bold">
                                    ...
                                    <span class="text-slate-700" x-text="doc.rating.toFixed(1)"></span>
        ...
        <div class="lg:col-span-1 space-y-6" id="booking-section">
            
            <!-- Contextual Booking Form -->
            <div class="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-slate-900/40">
                <div class="absolute top-0 right-0 w-64 h-64 bg-teal-500/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
                
                <div x-show="!showBookingForm" class="text-center py-12 relative z-10">
                    <div class="w-20 h-20 bg-white/5 border border-white/10 rounded-[2rem] flex items-center justify-center mx-auto mb-6 backdrop-blur-sm">
                        <svg class="w-10 h-10 text-teal-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    </div>
                    <h4 class="text-2xl font-black mb-3 leading-tight tracking-tight">Ready to book?</h4>
                    <p class="text-slate-400 font-bold text-sm leading-relaxed mb-8">Select a specialist from the national directory to begin your consultation request.</p>
                    <button @click="searchQuery = ''; selectedSpecialty = 'All'" class="text-[10px] font-black text-teal-400 uppercase tracking-widest hover:text-white transition-colors">Browse Directory →</button>
                </div>

                <div x-show="showBookingForm" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="relative z-10" style="display: none;">
                    ...
                    <div class="mb-10 p-6 bg-white/5 rounded-3xl border border-white/10 flex items-center gap-5">
                        <div class="w-14 h-14 bg-teal-500 rounded-2xl flex items-center justify-center font-black text-white text-xl shadow-lg shadow-teal-500/20">
                            <span x-text="selectedDoctor?.name.split(' ')[1][0]"></span>
                        </div>
                        <div>
            ...
            <div class="bg-teal-50 rounded-[2.5rem] p-8 border border-teal-100">
                <h4 class="text-xs font-black text-teal-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    Booking Policy
                </h4>
                <div class="space-y-4">
                    <div class="flex gap-4">
                        <div class="w-8 h-8 rounded-full bg-white text-teal-600 flex items-center justify-center flex-shrink-0 shadow-sm font-black text-xs">1</div>
                        <p class="text-xs text-teal-800/70 font-bold leading-relaxed">Appointments are subject to doctor's clinical availability and hospital hours.</p>
                    </div>
                    <div class="flex gap-4">
                        <div class="w-8 h-8 rounded-full bg-white text-teal-600 flex items-center justify-center flex-shrink-0 shadow-sm font-black text-xs">2</div>
                        <p class="text-xs text-teal-800/70 font-bold leading-relaxed">Emergency cases should be reported via the SOS system, not through standard booking.</p>
                    </div>
                </div>
            </div>
        </div>
