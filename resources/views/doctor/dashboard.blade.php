    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <svg class="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1 0V9m0 0h.01"></path></svg>
            ...
            @if($pendingApprovals->count() > 0)
                <div class="space-y-4">
                    @foreach($pendingApprovals as $appointment)
                        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-yellow-50 rounded-2xl border border-yellow-200">
                            <div>
                                <p class="font-semibold text-gray-800">{{ $appointment->patient->first_name }} {{ $appointment->patient->last_name }}</p>
                                <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($appointment->date)->format('M d, Y') }} • {{ $appointment->time_slot }}</p>
                                <p class="text-sm text-gray-500">{{ $appointment->hospital->name }}</p>
                            </div>
                            <div class="mt-3 md:mt-0 flex gap-2">
                                <form action="{{ route('doctor.appointment.approve', $appointment->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 transition-colors">Approve</button>
                                </form>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    <svg class="w-12 h-12 mx-auto text-gray-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1 0V9m0 0h.01"></path></svg>
                    <p>No pending appointment approvals right now.</p>
                </div>
            @endif
        </div>
